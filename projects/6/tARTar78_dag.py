...
with DAG('sentiment_analysis', start_date=datetime(2024, 5, 28), schedule_interval=None, catchup=False) as dag:
    ...
    feature_eng_train_task = SparkSubmitOperator(
        ...
        application_args=['--train-in', '/datasets/amazon/amazon_extrasmall_train.json', '--train-out', f'{base_dir}/tARTar78_train_out'],
        env_vars={
    ...
    feature_eng_test_task = SparkSubmitOperator(
        ...
        conn_id='spark_default',
        application=f'{base_dir}/feature_engineering.py',
        application_args=['--test-in', '/datasets/amazon/amazon_extrasmall_test.json', '--test-out', f'{base_dir}/tARTar78_test_out'],
        env_vars={
            'PYSPARK_PYTHON': '/opt/conda/envs/dsenv/bin/python'
        },
        spark_binary=spark_binary1
